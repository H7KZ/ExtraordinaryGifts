    ...
    public static void command(Player p) {
        ...
        if (Variables.Economy.Gift.enabled) {
            if (!GiftEconomy.Gift.checkBalance(p.getInventory().getItemInMainHand())) {
                p.sendMessage(ChatColor.RED + "You don't have enough balance to create a gift or you aren't holding it in the main hand! To see all possible ways of paying, type /gift economy.");
                return;
            }
